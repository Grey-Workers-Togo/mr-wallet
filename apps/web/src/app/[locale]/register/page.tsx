...
export default function RegisterPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      await apiClient.post('/auth/register', { email, password, baseCurrency });
      setRegistered(true);
    } catch (err) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-bg-secondary px-4">
      <Card className="w-full max-w-sm p-8 space-y-6">
        <CardHeader className="p-0 space-y-2">
          <CardTitle>{registered ? t('successTitle') : t('title')}</CardTitle>
          <CardDescription>{registered ? t('successMessage') : t('subtitle')}</CardDescription>
        </CardHeader>
