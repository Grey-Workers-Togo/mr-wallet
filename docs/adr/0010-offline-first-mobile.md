...
- **Reports exist in two forms, not two implementations.** The server keeps its aggregated SQL (RG-RP1, justified by concurrency and unbounded history); the device runs `packages/analytics-core`, a pure implementation that is *also* the oracle the server's SQL is tested against (`13-mobile-offline-first.md § 6`). The device therefore cannot drift from the reference — it executes it. What remains to maintain is one parity suite between the server's SQL and the reference, which is worth having anyway. This is a smaller cost than a straight port of the queries to SQLite would have been, and it holds only as long as each user's dataset stays small and fully replicated (RG-MD8).
