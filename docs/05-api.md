...
| GET | `/auth/:provider/callback` | Provider redirects the browser back here. Never returns JSON — always a 302: to `WEB_APP_URL/login/oauth?ticket=...` (already-linked or verified-email auto-link, see below), to `WEB_APP_URL/register/oauth?token=...` (brand-new identity, see below), or to `WEB_APP_URL/login?error=<code>` (see § 2 bis). |
...
| POST | `/auth/oauth/login-complete` | Finishes a returning-user OAuth login. Body: `ticket` (from the callback redirect above). Returns `accessToken` + `user` — no refresh cookie here (already set best-effort on the callback redirect itself). |
...
A returning-user login (already-linked provider, or an auto-linked verified email) never gets an access token on the callback redirect itself either — the callback is a plain HTTP 302, so an access token has nowhere safe to travel, and the refresh cookie it sets is cross-site relative to the web app's origin, which browsers that block third-party cookies (Safari ITP, Firefox ETP strict) simply refuse to store. The callback instead issues a short-lived, single-use `OAuthLoginTicket` and redirects to `/login/oauth?ticket=...`, a page that immediately exchanges it via `/auth/oauth/login-complete` for a real access token — no cookie involved in that exchange at all. The refresh cookie is still set on the callback redirect (best-effort, for browsers that do accept it, useful on later reloads).
