...
- **A returning login doesn't get its access token from the redirect either.** The callback is a plain HTTP 302 — an access token has nowhere safe to travel in it, and the refresh cookie set on that same redirect is cross-site relative to the web app's origin, which some browsers (Safari ITP, Firefox ETP strict) refuse to store by default; relying on it alone left the dashboard permanently unauthorized for those users on their very first load. Same fix as the signup case: an opaque, single-use, 5-minute `OAuthLoginTicket` travels in the redirect instead, exchanged via `POST /auth/oauth/login-complete` for the real access token with no cookie involved in that exchange.
